...
# define preprocessing transformations (common practice for diffusion model performance)
### This `transform` does two things: (1) converts the image to a PyTorch tensor, and (2) normalizes the image to have pixel values between [-1, 1]
transform = transforms.Compose([
    transforms.ToTensor(),  # convert images to PyTorch tensors
    transforms.Normalize((0.5,), (0.5,))  # normalize pixel values to [-1, 1]   
])

## Download the `train` split of the MNIST dataset, then apply our `transform` to each image
X = torchvision.datasets.MNIST('~/.pytorch/MNIST_data/', train=True, download=True, transform=transform)

batch_size = 128
train_dataloader = torch.utils.data.DataLoader(X, batch_size=128, shuffle=False)


# As a sanity check, let's view a random image from our dataset
C: int = 1 # number of channels
W: int = 28 # width of image (pixels)
H: int = 28 # height of image (pixels)
image: Float[torch.Tensor, "1 28 28"] = X[0][0]
label: int = X[0][1]
...
class DenoiseMLP(nn.Module):
    ...
    def forward(self, x, t):
        ...
        # from [N, 1, 28, 28] to [current_batch_size, 784]
        x_flat = x.view(x.shape[0], -1)  # flatten the 28x28 image to a vector of size 784
        ...
        #cat along dim=1 (the feature dimension), so h is (N,785) 
        h = torch.cat([x_flat, t_embed], dim=1)
        # Linear transform fc1: (N,785) * (785,128)^T + bias → (N,128).
...
for epoch in range(n_epochs):
   ...
   # idx = torch.randint(0, n_samples, (batch_size,))
    ...
    for batch_x0 in train_dataloader:
        x0 = batch_x0[0]
...
def sample(model, n_samples):
    x = torch.randn(n_samples, 784)  # start from pure noise
